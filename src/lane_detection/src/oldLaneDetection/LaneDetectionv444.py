...
class LaneDetection():
    ...
    def view_images(self):      # imshow a dönmeli
    ...
    def get_poly(self, src_img, dst_img):
        ...
        return [poly, draw_points], polyImg
    ...
    def find_lane(self, lanes_raw, lanesImg, mode):
        ...
        if(mode == "left"):
            self.laneLines_img, self.blendedIm = self.plot_lane_lines(neg_lines)
        if(mode == "right"):
            self.laneLines_img, self.blendedIm = self.plot_lane_lines(pos_lines)
        else:
            pass
        ...
        self.dilateImg = cv2.dilate(self.laneLines_img.copy(), kernel, iterations=2)
        ...
        lane_poly, self.img3 = self.get_poly(src_img=self.img2.copy(), dst_img=self.img2.copy())


        return lane_poly, blendedIm3
    ...
    def process_image(self):
        ...
        height, width = self.smoothedEdge.shape
        #   Left Mask
        center_t, center_b = ((width/2, 0), (width/2, height))
        l_vertices = np.array([[(0,0), (0, height), center_b, center_t]], dtype=np.int32)
        # create image only where mask and edge Detection image are the same
        left_img = cv2.bitwise_and(self.smoothedEdge.copy(), cv2.fillPoly(np.zeros_like(self.smoothedEdge.copy()) , l_vertices, 255))
        
        #   Right Mask
        r_vertices = np.array([[center_t, center_b, (width, height), (width, 0)]], dtype=np.int32)
        # create image only where mask and edge Detection image are the same
        right_img = cv2.bitwise_and(self.smoothedEdge.copy(), cv2.fillPoly(np.zeros_like(self.smoothedEdge.copy()) , r_vertices, 255))



        left_lanes, self.leftLines_img = self.hough_lines(left_img.copy())

        right_lanes, self.rightLines_img = self.hough_lines(right_img.copy())



        left_poly, blendedImL = self.find_lane(left_lanes, self.leftLines_img, "left")
        

        right_poly, blendedImR = self.find_lane(right_lanes, self.rightLines_img, "right")
        
        


        if(len(right_poly[1]) > len(left_poly[1])*2.5):
            
            print(left_poly[1][0:10])
            print("1")
            print(right_poly[1][0:10])
            #print(np.add(self.imshape[0]*np.ones_like(right_poly), -right_poly[1][1]))
            #left_poly[1] = cv2.flip(right_poly[1], 0)
            left_poly[1] = np.array([np.add(self.imshape[0]*np.ones_like(right_poly[1]), -right_poly[1][1])])
            print(left_poly[1][0:10])
            

        if(len(left_poly[1]) > len(right_poly[1])*2.5):
            
            print(right_poly[0:10])
            print("2")
            right_poly[1] = np.array([right_poly[1][0], np.add(self.imshape[0]*np.ones_like(left_poly), -left_poly[1][1])])
            print(right_poly[0:10])
        ...
        tfimg_l = np.split(self.img2, 2, axis=1)[0]
        


        tfimg_r = np.split(self.img2, 2, axis=1)[1]

        blendedImL = np.split(blendedImL, 2, axis=1)
        blendedImR = np.split(blendedImR, 2, axis=1)
        out_in = np.concatenate((blendedImL[0], blendedImR[1]), axis=1)

        self.img2 = np.concatenate((tfimg_l, tfimg_r), axis=1)

                
        self.img4, path = self.get_target_points(self.img2.copy(), [right_poly, left_poly])
            
        #self.view_images()


        return out_in, self.img4, path
