...
class LaneDetection():
    ...
    def process_image(self):
        ...
        #left_poly, left_draw_points, left_laneContours, left_tf_img = self.find_lane(left_lines, leftLines_img, "left")
        ...
        #right_poly, right_draw_points, right_laneContours, right_tf_img = self.find_lane(right_lines, rightLines_img, "right")
        ...
        if(len(right_draw_points) > len(left_draw_points)*2.5):
            ...
            left_draw_points = cv2.flip(right_draw_points, 0)
            #left_poly[1] = np.array(np.add(self.imshape[0]*np.ones_like(right_poly[1]), -right_poly[1][1])])
            
            

        if(len(left_poly) > len(right_draw_points)*2.5):
            
            right_draw_points = cv2.flip(left_draw_points, 0)
        ...
        out_in, path = self.get_target_points(tfimg.copy(), [right_poly, left_poly], [right_draw_points, left_draw_points])
        print(blendedIm.shape)      
        print(out_in.shape)    

        return  blendedIm, out_in, path
