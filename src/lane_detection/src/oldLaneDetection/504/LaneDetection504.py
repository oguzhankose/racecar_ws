...
class LaneDetection():
    ...
    def get_target_points(self, src, poly_list, draw_points_list):
        ...
        if(len(start) > 2):
            ...
            start[] = (int(polyval(self.st_point, poly_list[])), self.st_point)
            finish[] = (int(polyval(self.fi_point, poly_list[])), self.fi_point)
            median[] = (int(polyval(self.md_point, poly_list[])), self.md_point)
    ...
    def process_image(self, pr_img):
        ...
        cv2.imshow("blend", blendedImg_out)
        cv2.imshow("out", output_tf)
        cv2.waitKey(0)
